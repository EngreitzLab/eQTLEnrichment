...
# 		os.path.join(config["outDir"], "density.pdf")
		expand(os.path.join(config["outDir"], "variantLists/GTExVariantsForPrediction.{tissue}.tsv"), tissue=TISSUES)
...
rule testRule:
	...
	output:
		testOut = os.path.join(config["outDir"], "{variantLists/GTExVariantsForPrediction.{tissue}.tsv")
	run:
		shell(
				"""
				echo {{tissue}} > {output.testOut}	
				""")
