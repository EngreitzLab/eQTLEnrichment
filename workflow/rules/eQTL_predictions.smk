...
rule add_proximal_genes:
	...
	shell:
		...
		cat {input.variantsByTissue} | cut -f 1-4 | sort -k 1,1 -k2,2n | uniq | bedtools slop -i stdin -b 100000 -g {params.chrSizes} | bedtools intersect -wa -wb -a stdin -b {input.specificTSSFile} | cut -f 4,8 > {params.outDir}/{wildcards.method}/eGenePrediction/{wildcards.GTExTissue}.temp.100kbInterval.tsv
		...
		cat {input.variantsByTissue} | bedtools closest -t first -a stdin -b {input.specificGeneFile} | cut -f 1-4,6,7,12 | bedtools closest -t first -a stdin -b {input.specificTSSFile} | cut -f  1-7,11 > {params.outDir}/{wildcards.method}/eGenePrediction/{wildcards.GTExTissue}.temp.closest.tsv
