...
GTExNameKey = data.frame(GTExTissue=c("Cells_EBV-transformed_lymphocytes", "Liver", "Thyroid"), GTExName=c("EBV-transformed\nlymphocytes", "Liver", "Thyroid"))
```
...
```{r varPerTissue, echo=FALSE, fig.width=10}
# aggregate data
...
# aggregate data
  enr.all = read.table(tables[1], header=TRUE, stringsAsFactors = FALSE, fill = TRUE) %>% drop_na()
  enr.all$method = methods[1]
  if(length(methods)>1){
      for (i in 2:length(methods)){
        temp = read.table(file=tables[i], header=TRUE, stringsAsFactors = FALSE, fill=TRUE) %>% drop_na()
        temp$method = methods[i]
...
```{r heatMaps, echo=FALSE, results='asis', fig.height=12}
 ...
 # graph variant capture, prediction rate across all methods and tissues
  ...
  sr = ggplot(small.rates, aes(x=metric, y=value, fill=predictionMethod)) + 
    geom_bar(stat="identity", position="dodge", width=0.8) + 
     geom_text(aes(label = round(value,2)), vjust = -0.4, stat="identity", position=position_dodge(0.8)) +
    ...
    scale_fill_viridis(discrete=TRUE,option='viridis',name='Method') + ylim(0,0.2)
...
```{r ppvGraph, echo=FALSE, fig.width=9}
...
ppv = ggplot(df.ppv, aes(x=method, y=value, fill=predictionMethod)) +  
  ...
  facet_grid(GTExTissue~., scales='fixed') + theme_minimal() + xlab('') + ylab('Positive predictive value') + 
      theme(axis.text.x = element_text(angle=60,hjust=1), legend.position='none', axis.text.y=element_text(hjust=1)) + 
      scale_fill_viridis(discrete=TRUE,option='viridis') + ylim(0,1)
...
```{r predMetricsTable, results='asis', echo=FALSE, fig.width=7, fig.height=5}
 ...
 for (i in 1:nrow(df.table)){
   if (df.table$predictionMethod[i]=="Proximity"){
     if (df.table$predictionMethod[i]=="Positive predictive value"){
       df.table$predictionMethod[i] = "TSS within 100 kb"
     } else {
        df.table$predictionMethod[i] = df.table$metric[i]
        df.table$predictionMethod[i] = "Recall"
     }
   }
 }
...
cat(knitr::knit_print(reactable(df.metric, striped=TRUE, searchable=TRUE, filterable=TRUE,
  ...
  # cat(knitr::knit_print(reactable(df, striped=TRUE, searchable=TRUE, filterable=TRUE,
  #                                 defaultPageSize=5, resizable=TRUE,
  #         columns = list(GTExTissue = colDef(name = "GTEx tissue"),
  #                        predictionMethod = colDef(name = "Prediction method"),
  #                        metric = colDef(name = "Metric"),
  #                        value = colDef(name="Value", maxWidth=100,format=colFormat(digits=2))))))
  # 
  cat("  \n")
  cat("  \n")
...
## Sensitivities
The following table includes the prediction rate of a method in linking a variant to the correct eGene, given the set of variants that fall in enhancers for a given method. If a variant in this set is not predicted to be in an enhancer for the method being evaluated, it was assumed to be an incorrect prediction.
