...
boxplot.summary = ggplot(enr.all, aes(x = method, y = enrichment, fill =
                                        method)) +
  geom_boxplot() +
  theme_minimal() + ylab(enrLabel) + xlab('') +
  scale_fill_viridis(discrete = TRUE) +
...
small.rates$metric = plyr::revalue(
  small.rates$metric,
  c(
    "% variants in any enhancer" = "Overlaps predicted enhancer",
