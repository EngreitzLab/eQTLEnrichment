...
for (i in 1:length(methods)) {
  ...
  cat(knitr::knit_print(reactable(table.current, striped=TRUE, highlight=TRUE, searchable=TRUE, filterable=TRUE,
 ...
 # graph variant capture, prediction rate across all methods and tissues
  ...
  small.rates$metric = plyr::revalue(small.rates$metric, 
                                       c("% variants in any enhancer"="Overlaps predicted enhancer",
                                         "Prediction rate"="Overlaps predicted enhancer linked to correct gene"))
...
#    theme(text = element_text(size = rel(1)), axis.text.y=element_text(size=rel(0.75)), legend.text=element_text(size=rel(1))) +
...
## Prediction metrics
```{r predMetricsTable, results='asis', echo=FALSE}
  cat("  \n")
  cat("  \n")
  cat("  \n")

  cat(knitr::knit_print(reactable(df, striped=TRUE, highlight=TRUE, searchable=TRUE, filterable=TRUE,
                                  defaultPageSize=5, resizable=TRUE)))
...
## Sensitivities
```{r sensitivitiesTable, results='asis', echo=FALSE}
...
cat('test before\n')
  ...
  cat(knitr::knit_print(reactable(metrics, striped=TRUE, highlight=TRUE, searchable=TRUE, filterable=TRUE,
                                  defaultPageSize=5, resizable=TRUE)))
