...
main <- function() {
  option_list <- list(
    ...
    make_option(c("--distances"), type="character", default="1000 10000 100000 5000000")
  )
  
    opt = parse_args(OptionParser(option_list=option_list))
    outDir = opt$out; #outFile.pred = paste0(outDir, "/predictionRates.pdf"); outFile.PPV = paste0(outDir, "/PPV.pdf")
    tables = strsplit(opt$tables, " ") %>% unlist() %>% data.frame() %>% setNames(c("fName"))
    cpList = readRDS(opt$colors)
    distances = opt$distances %>% strsplit(" ") %>% unlist()
  ...
  for(i in 1:length(distances)){
    ...
    tables.filtered = dplyr::filter(tables, grepl(distances[i], tables$fName, fixed=TRUE))
    for (fileName in tables.filtered$fName){
    ...
    fOut = paste0("/predictionMetrics.under", distances[i], "bp.tsv")
    write.table(df, file=paste0(outDir, fOut), sep="\t", row.names=FALSE, col.names=TRUE, quote=FALSE)
  }
