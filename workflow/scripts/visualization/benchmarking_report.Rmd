...
to_order = dplyr::filter(nVariants, distance_bin=="n_all") %>% arrange(var_count)
nVariants$tissue = factor(nVariants$tissue, levels=to_order$tissue, ordered=TRUE)
nVariants$distance_label = factor(nVariants$distance_label, levels=dist$distance_label, ordered=TRUE)
...
- For distance stratified plots, eVariant-eGene distances were stratified at the following Kp values: `r distances`
...
for (i in 1:length(GTExTissues_matched)) {
  ...
  g = ggplot(data=df_line, aes(x=recall.linking, y=enrichment, color=key)) +
    ...
    ylim(c(0, 50)) +
    theme_classic() + theme(axis.text = element_text(size = 7), axis.title = element_text(size = 8),
	legend.position="bottom", legend.direction="horizontal", legend.text = element_text(size=7), legend.title=element_text(size=8))
  plot(g)
